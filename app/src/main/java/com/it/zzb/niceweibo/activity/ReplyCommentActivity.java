...
public class ReplyCommentActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.send_comment:
                ...
                break;
            case R.id.weibo_at:
                showToast(ReplyCommentActivity.this,"还未实现~",200);
                break;
            case R.id.weibo_topic:
                insertTopic();
                showToast(ReplyCommentActivity.this,"插入话题",200);
                break;
    ...
    public void sendComment(){
        ...
        if(TextUtils.isEmpty(content)) {
            showToast(ReplyCommentActivity.this,"评论内容不能为空",200);
            return;
        }
        ...
        long cid = Long.parseLong(comment.id);

        if (mAccessToken != null && mAccessToken.isSessionValid()) {

            commentsAPI.reply(cid,id,content,false,false,mListener);
    ...
    private RequestListener mListener = new RequestListener() {
        ...
        public void onComplete(final String response) {
            ...
            showToast(ReplyCommentActivity.this,"发送成功",200);

            // 发送成功后,设置Result结果数据,然后关闭本页面
